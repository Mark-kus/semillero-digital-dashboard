...
// Reducer
function classroomDataReducer(
  state: ClassroomDataState,
  action: ClassroomDataAction,
): ClassroomDataState {
  switch (action.type) {
    case "SET_LOADING":
    ...
    case "UPDATE_ADAPTED_DATA":
      return {
        ...
        studentProgress: adaptStudentProgress(
          state.students,
          state.courses,
          state.assignments,
          state.submissions,
        ...
        assignmentData: adaptAssignmentData(
          state.assignments,
          state.courses,
          state.submissions,
          state.students,
        ),
        recentActivity: adaptRecentActivity(
          state.assignments,
          state.submissions,
          state.courses,
          state.students,
...
export function ClassroomDataProvider({children}: {children: React.ReactNode}) {
  ...
  const loadSubmissions = useCallback(
    async (courseId?: string, assignmentId?: string) => {
      if (!authState.isAuthenticated) return;
      ...
      try {
        ...
        if (assignmentId && courseId) {
          ...
          const response = await fetch(
            `/api/classroom/submissions?courseId=${courseId}&assignmentId=${assignmentId}`,
          );
          ...
          for (const assignment of assignments) {
            const response = await fetch(
              `/api/classroom/submissions?courseId=${assignment.courseId}&assignmentId=${assignment.id}`,
            );
